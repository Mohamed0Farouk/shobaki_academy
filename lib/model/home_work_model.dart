...
class HomeWorkModel extends StatelessWidget {
  const HomeWorkModel({
    super.key,
    this.answer,
  ...
  Widget build(BuildContext context) {
    ...
    final HomeworkModelController controller = Get.put(
      HomeworkModelController(questionId),
      tag: questionId, // Add this tag
    );
    final HomeworkController homeworkController = Get.find();

    // Set the initial state of the controller
    if (answer != null) {
      controller.selectedAnswer.value = answer!;
      homeworkController.answer = answer;
    }
                            ...
                            onSelected: controller.flag.value
                                ...
                                : (value) {
                                    if (value) {
                                      ...
                                      homeworkController.answer =
                                          answers[index];
                                    } else {
                                      controller.resetAnswer();
                                      homeworkController.answer = null;
                                    }
                                  },
                          ),
                        ),
